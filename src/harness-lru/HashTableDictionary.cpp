...
std::string HashTableDictionary::getOccupancyMap(int lineWidth) const {
    std::string map;
    map.reserve(TABLE_SIZE + (TABLE_SIZE / lineWidth) + 1);
    
    for (std::size_t i = 0; i < TABLE_SIZE; i++) {
        // 1 = USED or DELETED, 0 = AVAILABLE
        map += (hashTableMask.at(i) == AVAILABLE) ? '0' : '1';
...
void HashTableDictionary::clear() {
    ...
    hashTableMask.resize(TABLE_SIZE, AVAILABLE);
     ...
     maxValuesInTable = 0;
     beforeCompactionMap.clear();
     afterCompactionMap.clear();
...
void HashTableDictionary::compactTable() {
    ...
    beforeCompactionMap = getOccupancyMap();
    ...
    */
    ...
    afterCompactionMap = getOccupancyMap();
    /*
...
void HashTableDictionary::printCounts() {
    ...
    for(std::size_t i = 0; i < TABLE_SIZE; i++) {
        if (hashTableMask.at(i) == USED) {
            ...
            int prevIdx = 0;
            std::cout << std::setw(5) << ' ';
            for(std::size_t j = 0; j < probePath.at(i).size(); j++) {
